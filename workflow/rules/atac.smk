...
rule archr_build:
    ...
    """
    input:
        frags = expand("results/{sample}/fetch/fragments.tsv.gz", sample=samples),
        frag_ind = expand("results/{sample}/fetch/fragments.tsv.gz.tbi", sample=samples),
    output:
        ...
        metadata = "results_merged/atac/archr_metadata.tsv"
    params:
