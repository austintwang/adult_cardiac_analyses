...
def load_amulet_data(barcodes, amulet_data_path, amulet_overlaps_path):
    records = {}
    ...
    with open(amulet_overlaps_path) as f:
        h = f.readline().rstrip('\n').split('\t')
        barcode_ind = h.index("Cell Id")
        overlap_ind = h.index("Number of Overlaps")
        ...
        for line in f:
            ...
            overlaps = int(entries[overlap_ind])

            records.setdefault(barcode, {
                "amulet_pval": "NA",
                "amulet_qval": "NA"
            })
            records[barcode]["num_overlaps"] = overlaps
...
def main(sample_name, metadata_dir, final_data_path, amulet_data_dir, bc_atac_path, bc_rna_path, barcodes_path, out_path):
    ...
    amulet_overlaps_path = os.path.join(amulet_data_dir, "OverlapSummary.txt")
    records_amulet = load_amulet_data(barcodes, amulet_data_path, amulet_overlaps_path)
    for k, v in records_amulet.items():
        records[k].update(v)

    for v in records.values():
        v["sample"] = sample_name
        v["cell_id_seurat"] = f"{sample_name}_{v['barcode_rna']}"
