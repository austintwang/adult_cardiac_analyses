...
plt <- plot_fn(proj, "cell_types_split", "umap", stallion) + labs(title="Cell Types (Full-Resolution)")
ggsave(output_paths[["umap_full"]], plt, device = "pdf", width = 10, height = 7)

plt <- plot_fn(proj, "cell_types_merged", "umap", stallion) + labs(title="Cell Types (Merged)")
ggsave(output_paths[["umap_merged"]], plt, device = "pdf", width = 10, height = 7)
