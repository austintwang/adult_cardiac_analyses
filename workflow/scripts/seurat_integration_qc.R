...
plt <- FeaturePlot(proj_merged, reduction = "umap", features = "log_counts") + labs(title="Post-Harmony log10 UMIs")
...
plt <- FeaturePlot(proj_merged, reduction = "umap", features = doubletfinder_col) + labs(title="Post-Harmony Doubletfinder pANN")
ggsave(output_paths[["umap_doubletfinder"]], plt, device = "pdf", width = 10, height = 7)

proj$amulet_nlp <- -log10(proj$amulet_pval)
plt <- FeaturePlot(proj_merged, reduction = "umap", features = "log_counts") + labs(title="Post-Harmony Amulet -log10 p-Value")
